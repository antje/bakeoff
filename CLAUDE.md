...
- The verdict is computed, never chosen: cheapest cell by cost per correct call among those within `--tolerance` (default 0.05) of the best gate rate, above the trivial baseline, and inside `--ttft-budget` if one was given. The report names the cells it dropped and why.
